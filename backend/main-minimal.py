...
# Simplified models using dict instead of Pydantic
app = FastAPI(
    title="Abide: Christian AI Companion (Minimal)",
    description="A minimal version for testing without external dependencies",
    version="1.0.0"
)
...
async def root():
    return {
        "message": "Welcome to Abide: Christian AI Companion (Minimal Version)",
        "version": "1.0.0",
        "status": "healthy"
    }
...
async def health_check():
    return {"status": "healthy", "service": "abide-backend-minimal"}

@app.post("/api/v1/feel")
async def process_feeling(request: dict):
    ...
    try:
        ...
        topic = classify_feeling(text)
        
        # Get relevant Bible verses
        verses_data = TOPIC_VERSES.get(topic, TOPIC_VERSES["comfort"])
        verses = random.sample(verses_data, min(2, len(verses_data)))
...
@app.post("/api/v1/devotion")
async def generate_devotion(request: dict):
    ...
    try:
        ...
        # Determine theme
        if not theme:
            theme = random.choice(list(RESPONSE_TEMPLATES.keys()))
        
        # Get relevant Bible verses
        verses_data = TOPIC_VERSES.get(theme, TOPIC_VERSES["comfort"])
        scriptures = random.sample(verses_data, min(3, len(verses_data)))
        ...
        # Mock YouTube video with realistic data
        video_data = {
            "peace": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Perfect Peace - Lauren Daigle",
                "channelTitle": "Lauren Daigle",
                "thumbnailUrl": "https://via.placeholder.com/320x180/10B981/FFFFFF?text=Perfect+Peace",
                "duration": 285,
                "description": "A beautiful worship song about finding God's perfect peace."
            },
            "hope": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Hope in Front of Me - Danny Gokey",
                "channelTitle": "Danny Gokey",
                "thumbnailUrl": "https://via.placeholder.com/320x180/F59E0B/FFFFFF?text=Hope+in+Front+of+Me",
                "duration": 320,
                "description": "An encouraging song about hope and God's promises."
            },
            "comfort": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Comforter - Francesca Battistelli",
                "channelTitle": "Francesca Battistelli",
                "thumbnailUrl": "https://via.placeholder.com/320x180/8B5CF6/FFFFFF?text=Comforter",
                "duration": 295,
                "description": "A comforting song about God's presence in difficult times."
            },
            "strength": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Stronger - Mandisa",
                "channelTitle": "Mandisa",
                "thumbnailUrl": "https://via.placeholder.com/320x180/EF4444/FFFFFF?text=Stronger",
                "duration": 310,
                "description": "An empowering song about finding strength in God."
            },
            "anxiety": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Cast My Cares - Finding Favour",
                "channelTitle": "Finding Favour",
                "thumbnailUrl": "https://via.placeholder.com/320x180/06B6D4/FFFFFF?text=Cast+My+Cares",
                "duration": 275,
                "description": "A calming song about giving your worries to God."
            },
            "loneliness": {
                "videoId": "dQw4w9WgXcQ",
                "title": "Never Alone - BarlowGirl",
                "channelTitle": "BarlowGirl",
                "thumbnailUrl": "https://via.placeholder.com/320x180/84CC16/FFFFFF?text=Never+Alone",
                "duration": 300,
                "description": "A reassuring song about God's constant presence."
            }
        }
        
        video = video_data.get(theme, video_data["comfort"])
